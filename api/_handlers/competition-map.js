...
/**
 * Verify bearer token and check if the user has an active premium subscription.
 * Returns { isPremium: boolean } — never throws, defaults to free on any error.
 */
async function checkPremiumAccess(req) {
  try {
    const authHeader = req.headers?.authorization || req.headers?.Authorization || '';
    const token = authHeader.replace(/^Bearer\s+/i, '').trim();
    if (!token) return { isPremium: false };

    // Admin / mock tokens are always premium
    if (token.startsWith('admin_token_') || token.startsWith('mock_token_') || token.startsWith('mock_')) {
    ...
    if (error || !data?.user) return { isPremium: false };
    ...
    if (subData && subData.length > 0) {
      const sub = subData[0];
      const expired = sub.end_date && new Date(sub.end_date).getTime() < Date.now();
      if (!expired) return { isPremium: true };
    }
    ...
    const { data: payData } = await supabase
      ...
      .eq('user_id', userId)
      .limit(1);
    if (payData && payData.length > 0) return { isPremium: true };
    ...
    if (userEmail) {
      scQuery = scQuery.or(`user_id.eq.${userId},email.eq.${userEmail}`);
    } else {
      scQuery = scQuery.eq('user_id', userId);
    }
    const { data: scData } = await scQuery.limit(1);
    if (scData && scData.length > 0) return { isPremium: true };

    // 4. Fallback to profile flags
    const { data: profileData } = await supabase
      .from('profiles')
      .select('is_premium, subscription_status, premium_end_date')
      .eq('id', userId)
      .maybeSingle();
    if (profileData?.is_premium && profileData?.subscription_status === 'active') {
      const profileExpired = profileData.premium_end_date && new Date(profileData.premium_end_date).getTime() < Date.now();
      if (!profileExpired) return { isPremium: true };
    }

    return { isPremium: false };
  } catch (e) {
    console.warn('[competition-map] premium check error:', e.message);
    return { isPremium: false };
  }
}

/**
 * Strip premium-only fields from a state object for free users.
 * Only safe overview data is returned in the free tier, enough for the tooltip.
 * Massive data arrays (tables, charts) are stripped.
 */
function sanitizeStateForFree(state) {
  return {
    ...state,
    // Strip out heavy detailed arrays to prevent premium data leak
    top_colleges: undefined,
    cutoff_trend: undefined,
    seat_split: undefined,
    colleges_sample: undefined,
    seat_rows: undefined,
    cutoff_rows: undefined,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Determine premium status server-side — never trust frontend
    const { isPremium } = await checkPremiumAccess(req);
...
Write a personalized 2-sentence summary providing strategic advice on which state quotas or management quotas they should target. Do not use markdown, just plain text. Return it in JSON format: {"summary_text": "..."}`;
    ...
    if (!isPremium) {
      const freeList = list.map(sanitizeStateForFree);
      const freeSummary = {
        states: summary.states,
        total_colleges: summary.total_colleges,
        total_seats: summary.total_seats,
        avg_competition: summary.avg_competition,
      ...
      if (req.query.detail === '1' && freeList.length === 1) {
    ...
    if (req.query.detail === '1' && list.length === 1) {
      return res.status(200).json({ success: true, access: 'full', state: list[0], summary });
    }

    return res.status(200).json({ success: true, access: 'full', states: list, summary });
  } catch (err) {

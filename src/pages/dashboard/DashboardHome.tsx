...
export default function DashboardHome() {
  ...
  return (
    <div className="max-w-[1080px] mx-auto w-full space-y-5 pb-8">
      ...
      {/* Applications */}
      <section className={`rounded-[22px] border overflow-hidden ${card}`}>
        <div
          className={`flex items-center justify-between px-5 py-4 border-b ${
            dark ? 'border-white/[0.06]' : 'border-[#eef0f3]'
          }`}
        >
          <div>
            <h3 className="font-bold">Your counsellings</h3>
            <p className={`text-xs font-medium mt-0.5 ${dark ? 'text-white/40' : 'text-[#9ca3af]'}`}>
              Live tracks from your applications
            </p>
          </div>
          ...
          {loading ? (
            <div className="grid sm:grid-cols-2 gap-3">
              {[1, 2].map((i) => (
                <div
                  key={i}
                  className={`h-28 rounded-2xl animate-pulse ${dark ? 'bg-white/5' : 'bg-[#f3f4f6]'}`}
                />
              ))}
            </div>
          ) : (summary?.applications || []).length > 0 ? (
            <div className="grid sm:grid-cols-2 gap-3">
              {(summary?.applications || []).slice(0, 4).map((app) => (
                <Link
                  key={app.id}
                  ...
                  className={`group rounded-2xl border p-4 transition-all hover:-translate-y-0.5 ${
                    dark
                      ...
                      : 'border-[#eef0f3] bg-[#fafbfc] hover:border-orange-200 hover:bg-white'
                  }`}
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <p className="font-bold text-[14px] leading-snug truncate">
                      {app.name || 'Untitled counselling'}
                    </p>
                    <span
                      className={`shrink-0 inline-flex text-[10px] font-bold uppercase tracking-wide px-2 py-1 rounded-md border ${statusStyle(
                        app.status,
                        dark
                      )}`}
                    >
                  ...
                  {app.external_id && (
                    <p className={`text-[11px] font-semibold mb-1 ${dark ? 'text-white/35' : 'text-[#9ca3af]'}`}>
                      {app.external_id}
                    </p>
                  )}
                  <p className={`text-xs font-medium line-clamp-2 ${dark ? 'text-white/45' : 'text-[#6b7280]'}`}>
                    {app.notes || 'Open applications to update status and notes.'}
                  </p>
                </Link>
              ))}
            </div>
            ...
            <Link
              to="/dashboard/counselling"
              className={`flex flex-col items-center justify-center min-h-[140px] rounded-2xl border border-dashed gap-2 transition-colors ${
                dark
                  ? 'border-white/15 text-white/45 hover:bg-white/[0.03]'
                  : 'border-[#d1d5db] text-[#9ca3af] hover:border-orange-300 hover:bg-orange-50/40'
              }`}
            >
              <CalendarCheck2 className="w-6 h-6 text-orange-500" />
              <span className="text-sm font-semibold">Add your first counselling track</span>
              <span className="text-xs">NEET UG · PG · State</span>
            </Link>
          )}
        </div>
      </section>
